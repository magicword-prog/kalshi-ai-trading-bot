"""
Anthropic client for AI-powered trading decisions.
Interfaces with Claude models through the Anthropic SDK for market analysis and trading strategies.
"""
...
class AnthropicClient(TradingLoggerMixin):
    """
    Anthropic client for AI-powered trading decisions.
    Uses Claude models for market analysis and trading strategy.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, db_manager=None):
        ...
        """
        self.api_key = api_key or settings.api.anthropic_api_key
    ...
    async def _handle_resource_exhausted_error(self, error_msg: str):
        """Handle Anthropic API resource exhausted errors."""
        self.logger.error(
    ...
    def _calculate_cost(self, input_tokens: int, output_tokens: int, model: Optional[str] = None) -> float:
        ...
        # Claude Opus pricing: $15/MTok input, $75/MTok output
        elif "opus" in model:
    ...
    async def search(self, query: str, max_length: int = 300) -> str:
        """
        Perform a search using Claude's knowledge.
        Since Anthropic doesn't have a built-in live search like xAI,
        this uses Claude's training data to provide context.
        """
        try:
            # Check cache first
            cache_key = f"{query[:50]}:{max_length}"
            if cache_key in self._search_cache:
                self.logger.debug("Returning cached search result", query=query[:50])
                return self._search_cache[cache_key]

            self.logger.debug(
                "Starting Anthropic knowledge search",
                query=query[:50],
                max_length=max_length
            )

            search_prompt = f"""Find current, relevant information about: {query}
...
Provide a brief, factual summary under {max_length // 2} words. If no current information is available, clearly state that."""
            ...
            if response_text:
                ...
                result = self._truncate_news_summary(response_text, max_length)
    ...
    def _truncate_news_summary(self, text: str, max_length: int) -> str:
        """Truncate text to approximate character length."""
    ...
    def _get_fallback_context(self, query: str, max_length: int) -> str:
        ...
        if "temp" in query.lower() and ("LA" in query or "Philadelphia" in query):
            ...
            return "Movie ratings and reviews vary based on critic and audience feedback. Rotten Tomatoes scores depend on the number and sentiment of reviews received. [Fallback: Search unavailable]"
        elif "release" in query.lower() and "album" in query.lower():
            return "Music release dates are typically announced by artists or labels through official channels. Release schedules can change based on various factors. [Fallback: Search unavailable]"
        else:
            ...
            try:
                if isinstance(close_time, str):
                    for fmt in ["%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S"]:
                        try:
                            close_dt = dt_module.datetime.strptime(close_time, fmt)
                            break
                        except ValueError:
                            continue
                    else:
                        close_dt = dt_module.datetime.now()
                ...
                if close_dt.tzinfo is None:
                    close_dt = close_dt.replace(tzinfo=dt_module.timezone.utc)
                if now.tzinfo is None:
                    now = now.replace(tzinfo=dt_module.timezone.utc)
    ...
    async def _make_completion_request(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Anthropic expects messages as list of dicts with role/content
                ...
                if not response_content or not response_content.strip():
                    ...
                    if attempt < max_retries - 1:
                        ...
                        if model_to_use == self.primary_model:
                            ...
                            fallback_result = await self._try_fallback_model(messages, temperature, max_tokens)
                            if fallback_result:
                ...
                self.logger.debug(
                    "Anthropic completion request successful",
                    model=model_to_use,
                ...
                self.logger.warning(
                    f"Rate limit on attempt {attempt + 1}: {str(e)}",
                    model=model_to_use,
                    attempt=attempt + 1,
                )
                if attempt < max_retries - 1:
    ...
    async def _try_fallback_model(
        ...
        try:
            ...
            if response_content and response_content.strip():
                input_tokens = response.usage.input_tokens
                output_tokens = response.usage.output_tokens
                cost = self._calculate_cost(input_tokens, output_tokens, fallback_model)
